			...
			foreach(const Neighbor &tmpVar, nbF(I)) {
				...
				for(map<size_t, int>::const_iterator iter=factor(I).counts()[tmpVar.iter].begin(); iter!=factor(I).counts()[tmpVar.iter].end();iter++,pos++) {
					if (tmpVar == i) {
						...
						for( IndexFor k( Var(sortedPos, var(i).states()), gndVarSet ); k.valid(); ++k ) {
...
double CBP::run() {
    ...
    for( _iters=0; _iters < props.maxiter && maxDiff > props.tol; ++_iters ) {
        ...
        if( props.verbose >= 3 )
            cout << Name << "::run:  maxdiff " << maxDiff << " after " << _iters+1 << " passes (" << toc() - tic << " secs.) " << endl;
    }
    ...
    if( props.verbose >= 1 ) {
        if( maxDiff > props.tol ) {
            if( props.verbose == 1 )
                cout << endl;
                cout << Name << "::run:  WARNING: not converged within " << props.maxiter << " passes (" << toc() - tic << " seconds)...final maxdiff:" << maxDiff << endl;
        } else {
            if( props.verbose >= 3 )
                cout << Name << "::run:  ";
...
void CBP::compress() {
	...
	CFactorGraph liftedFg = _compressAlg->compress((CFactorGraph &)(*this));
